...
class Solution:
    ...
    def wiggleMaxLength(self, nums: List[int]) -> int:
        ...
        count = 2 if then != 0 else 1
        for i in range(2, len(nums)):
            now = self.__signum(nums[i] - nums[i - 1])
            if now != 0:
                if now != then:
                    count += 1
                then = now
