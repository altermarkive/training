...
DFS\@: RECURSIVE versus BFS\@: FIFO\linebreak
...
\item[INT\@:] \( KEY\mod{M} \) where \( \left\{M: \text{MERSENNE PRIME}\right\} \) \linebreak\% IMPLEMENTATION CAN BE FAST\linebreak
\item[STRING\@:] \( (\sum{str[i] << b})\mod{M} \) \linebreak b \textendash{} NUMBER OF CHARACTER CODING BITS\linebreak
\end{description}
RESOLUTION (constant if enough buckets, linear within a bucket)\@:
\begin{itemize}
\item SEPARATE CHAINING (LISTS)\linebreak(OPEN HASHING, CLOSED ADDRESSING)
\item LINEAR PROBING (ARRAY)\linebreak(CLOSED HASHING, OPEN ADDRESSING)
\end{itemize}
}
...
\href{https://leetcode.com/problems/min-stack/}{leetcode.com/problems/min-stack/}\linebreak
\href{https://leetcode.com/problems/kth-largest-element-in-an-array/}{leetcode.com/problems/kth-largest-element-in-an-array/}\linebreak
}
\flashcard{32}{flushleft}{
...
\flashcard{42}{center}{
\textbf{RUNNING MEDIAN} \textendash{} GREATER (SMALLER) HALF OF THE NUMBERS IN MIN (MAX) HEAP\@; INSERT INCOMING INTO HEAP DEPENDING ON COMPARISON WITH CURRENT MEDIAN\@; REBALANCE IF NECESSARY\@; MEDIAN IS ONE OF ROOTS OR THEIR AVERAGE\@; REMOVE OUTGOING SIMILAR
}
...
(X \textendash{} pointer size in bytes\@; matrix is packed \textendash{} 8 booleans per byte)\linebreak
...
\flashcard{36}{center}{
Java \textendash{} red-black tree\@: TreeMap\linebreak
...
\begin{itemize}
\item (\textbf{shuffle} first or sample for pivot \textendash{} median as pivot helps\@; \href{https://leetcode.com/problems/shuffle-an-array/}{leetcode.com/problems/shuffle-an-array/})
\item PICK PIVOT
\item REORDER WITH RESPECT TO PIVOT
...
TIME \( O(N) \)\linebreak
AVERAGE MESSAGE \( O(N\log{N}) \)\linebreak
...
UNBOUNDED \textendash{} SAME ITEM CAN BE USED \(>\) ONCE\linebreak
MAXIMIZE THE VALUE SUM BUT KEEP WEIGHT UP TO A LIMIT\linebreak
SOLUTION\@: BOTTOM-UP DYNAMIC PROGRAMMING\linebreak
\ \\
PSEUDO POLYNOMIAL TIME (LIKE TESTING IF N IS PRIME) \textendash{} EXPONENTIAL IN LENGTH (NUMBER OF DIGITS) OF INPUT\@; POLYNOMIAL IN NUMERIC VALUE OF INPUT
}
...
\flashcard{40}{flushleft}{
\textbf{EXACT ALGORITHM} HELD-KARP \( O(N^{2}2^{N}) \)\linebreak
\textbf{APPROXIMATE ALGORITHM}
\begin{itemize}
\item \textbf{CONSTRUCTIVE HEURISTICS} GREEDY (NN \textendash{} 25\% of optimal), MINIMUM SPANNING TREE BASED ALGORITHMS (CHRISTOFIDES ALGORITHM)
...
\flashcard{60}{center}{
PROCESS VERSUS THREAD\linebreak
CONTEXT SWITCH
}
\flashcard{38}{flushleft}{
\textbf{THREAD}\@: \underline{PART OF PROCESS}, \underline{SHARED MEMORY}, \underline{ONLY STACK \& REGISTERS SAVED}, CTX SWITCH \textendash{} SMALLER CACHE IMPACT
\textbf{PROCESS}\@: \underline{INDEPENDENT}, \underline{SEPARATE MEMORY (MAP)}, STATE / IPC (FILE HANDLES, DEVICE HANDLES, SOCKETS), CTX SWITCH \textendash{} MEMORY MAP SWITCH, BIGGER CACHE IMPACT (SOME ARCHITECTURES MUST FLUSH), POSSIBLY STATE SWITCH, PAGING\linebreak
}
...
\flashcard{44}{center}{
TWO OR MORE PROCESSES WAIT FOR THE OTHERS TO RELEASE THE RESOURCE\linebreak
\ \linebreak
...
\flashcard{44}{flushleft}{
SLEEPLOCK \textendash{} SPINLOCK\linebreak
\linebreak
TEST-AND-SET \textendash{} FETCH-AND-ADD \textendash{} CMP-AND-SWAP\linebreak
\linebreak
\textbf{MUTEX}\@: 2 STATE\linebreak
\textbf{SEMAPHORE}\@: COUNTS NO OF UNITS OF RESOURCE AVAILABLE\linebreak
\textbf{MONITOR}\@: OBJECTS WHERE METHODS ARE MUTUALLY EXCLUSIVE
}
...
\item MULTILEVEL FEEDBACK QUEUE\@: 1 \textendash{} PREFERENCE TO SHORT OR I/O, 2 \textendash{} MULTIPLE FIFO, 3 \textendash{} STARTS AT END OF TOP FIFO, 4 \textendash{} (NORMALLY) TO THE END OF QUEUE (VOLUNTARY), 5 \textendash{} USES UP QUANTUM \textendash{} PREEMPT AND LOWER QUEUE
\end{itemize}
}
...
\flashcard{44}{center}{
compareTo \textendash{} lexi, ignoreCase, starts, endsWith, equals, indexOf, lastIndexOf, matches \textendash{} regex, replaceAll \textendash{} regex, replaceFirst \textendash{} regex, split, substring, toUpperCase, trim
}
...
\flashcard{44}{center}{
sorted([\ldots], key=lambda item: item[1], reverse=True)
}
...
\ \\
\textbf{VARIATION WITH REPETITIONS (EQUIVALENT GROUPS REPEAT)}\linebreak
\( N^{K} \)\linebreak
