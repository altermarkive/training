...
fn subsets_with_dup(nums: Vec<i32>) -> Vec<Vec<i32>> {
    ...
    nums.sort_unstable();
    subsets(&nums, 0, Vec::new(), &mut result);
    result
}

fn subsets(nums: &[i32], offset: usize, current: Vec<i32>, list: &mut Vec<Vec<i32>>) {
    list.push(current.clone());
    let mut i = offset;
    while i < nums.len() {
        let count = 1;
        for j in (i + 1)..nums.len().min(i + count) {
            if nums[j - 1] != nums[j] {
                break;
            }
            count += 1;
        ...
        for _ in 0..count {
            current.push(nums[i]);
            subsets(nums, i + count, current.clone(), list);
        }
        for _ in 0..count {
            current.pop();
        }
