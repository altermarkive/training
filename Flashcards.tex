...
% pdflatex -halt-on-error -interaction=nonstopmode Flashcards.tex
...
\begin{description}
\item[FLOAT:] \( \frac{KEY - MIN}{MAX - MIN}M \)\linebreak
\item[INT:] \( KEY\mod{M} \) where \( \left\{M: \text{MERSENNE PRIME}\right\} \) \linebreak\% IMPL. CAN BE FAST\linebreak
\item[STRING:] \( \sum{128^{i}str[i]}\mod{M} \) \linebreak 128 - DROP NONCODING BITS\linebreak
\end{description}
RESOLUTION:
\begin{itemize}
...
(X depends on pointer size; matrix is packed)\linebreak
...
\flashcard{60}{center}{
AVL TREE\linebreak
(SELF-BALANCING BST)\linebreak
BALANCING\linebreak
\ \\
(Java - red-black: TreeMap)
}

\flashcard{36}{center}{
SEARCH/INSERT/DELETION - WORST/AVG:\linebreak
\( O(N\log{N}) \)\linebreak
\linebreak
DECORATE NODES WITH SUBTREE HEIGHT\linebreak
\linebreak
AFTER INSERTION/DELETION GO UPWARDS TO ROOT AND ROTATE IF HEIGHTS DIFFER BY \(>\) 1 (AND RECOMPUTE HEIGHTS), LEFT CASE:
\begin{tabular}{ccc}
\xymatrix@-1pc{
  &                                     & *+[o][F-]{5} \ar@{-}[dl]\ar@{-}[dr]               &   \\
  & *+[o][F-]{3} \ar@{-}[dl]\ar@{-}[dr] &                                                   & D \\
A &                                     & *+[o][F-]{4} \ar@{-}[dl]\ar@{-}[dr]\ar@/_1pc/[ul] &   \\
  & B                                   &                                                   & C \\
}
&
\xymatrix@-1pc{
  &                                     &                                                   & *+[o][F-]{5} \ar@{-}[dl]\ar@{-}[dr] &   \\
  &                                     & *+[o][F-]{4} \ar@{-}[dl]\ar@{-}[dr]\ar@/^1pc/[ur] &                                     & D \\
  & *+[o][F-]{3} \ar@{-}[dl]\ar@{-}[dr] &                                                   & C                                   &   \\
A &                                     & B                                                 &                                     &   \\
}
&
\xymatrix@-1pc{
  &                                     &   &*+[o][F-]{4} \ar@{-}[dll]\ar@{-}[drr] &   &                                     &   \\
  & *+[o][F-]{3} \ar@{-}[dl]\ar@{-}[dr] &   &                                      &   & *+[o][F-]{5} \ar@{-}[dl]\ar@{-}[dr] &   \\
A &                                     & B &                                      & C &                                     & D \\
}
\end{tabular}
}
...
\flashcard{36}{center}{
\textbf{INSERTION:} 1. NORMALLY; 2. BALANCE
\textbf{DELETION:} 1. IF LEAF THEN REMOVE; 2. IF ONLY CHILD THEN REMOVE AND BIND; 3. OTHERWISE RECONNECT SPLICING SUCCESSOR (OR PREDECESSOR); 3. BALANCE\linebreak
\linebreak
SUCCESSOR CASE (SUP - SUPREMUM):\linebreak
\begin{tabular}{cc}
\xymatrix@R-1.5pc{
      &                          &                                           & *+[o][F-]{X} \ar@{-}[dll] \\
      & \ar@{--}[dl]\ar@{--}[dr] &                                           &                           \\
\dots &                          & *+[o][F-]{SUP} \ar@{-}[dl]\ar@/_1pc/[uur] &                           \\
      & \dots                    &                                           &                           \\
}
&
\xymatrix@R-1.5pc{
...
\begin{itemize}
\item (\textbf{shuffle} first or sample for pivot)
\item PICK PIVOT
...
\flashcard{55}{flushleft}{
\begin{description}
\item[O] bounded above (up to constant factor) asymptotically
\item[o] dominated asymptotically (note: every constant)
\item[\(\Theta\)] bounded both above and below asymptotically
\item[\(\Omega\)] bounded below by g asymptotically
\end{description}
}
...
\ \\
\(P \neq NP\):\linebreak
\(P \subset NP \land NP-COMPLETE \equiv NP \cap NP-HARD\)\linebreak
...
\flashcard{38}{center}{
NUMBER OF EACH SO WEIGHT\(<=\)W \& VALUE MAX\linebreak
BOTTOM-UP DYN. PROG:\linebreak
W - INTEGERS: UNBOUNDED (SAME ITEM \(>\) ONCE): \( max_{wi<=w}(m[w-1],max(vi,m[w-wi]) \)\linebreak
\ \\
...
\flashcard{26}{flushleft}{
\fontfamily{qcr}\selectfont
\begin{enumerate}
\item[] \# closure: function w/ an env.
\item[] def counter():
\begin{enumerate}
\item[] x = 0
\item[] def increment(y):
\begin{enumerate}
\item[] nonlocal x \# \( \leftarrow \) free variable, to be closed
\item[] x += y
\item[] print(x)
\end{enumerate}
\item[] return increment \# \( \leftarrow \) closure
\end{enumerate}
\end{enumerate}
}
