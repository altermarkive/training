...
struct LC219ContainsDuplicateII {
    k: usize,
}

impl LC219ContainsDuplicateII {
    fn new(k: usize) -> Self {
        Self { k }
    }

    fn contains_nearby_duplicate(&self, nums: &[i32], k: usize) -> bool {
        ...
        for &num in nums.iter().take(k + 1) {
            ...
            if queue.len() > k {
                seen.remove(&queue.pop_front().unwrap());
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_57_and2() {
        let mut lc = LC219ContainsDuplicateII::new(2);
        assert!(!lc.contains_nearby_duplicate(&[0, 5, 7], 2));
    }

    #[test]
    fn test_575_and2() {
        let mut lc = LC219ContainsDuplicateII::new(2);
        assert!(lc.contains_nearby_duplicate(&[0, 5, 7, 5], 2));
    }

    #[test]
    fn test_5705_and2() {
        let mut lc = LC219ContainsDuplicateII::new(2);
        assert!(!lc.contains_nearby_duplicate(&[0, 5, 7, 10, 5], 2));
    }
}
```